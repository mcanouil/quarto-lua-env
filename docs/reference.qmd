...
| --- | --- | --- | --- |
| `json` | boolean | `false` | `true` writes the export, `false` writes nothing. |
| `json-file` | string | `lua-env.json` | Path of the export, read only when `json` is `true`. |
...
A relative `json-file` path is resolved against the directory of the document being rendered, not the project root and not the output directory.
`json: true` therefore drops `lua-env.json` next to the source file, where it is a build artefact to ignore rather than something the site publishes.
...
`json` is a boolean, and `json-file` names the file. They were one option once, which meant `json: no` wrote a file called `no` rather than disabling the export.
...
The file is written as a single line, with no indentation and no guaranteed key order.
The tree below is laid out for reading:
